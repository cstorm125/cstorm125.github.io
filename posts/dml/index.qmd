...
$$
\left. \frac{\partial}{\partial \eta} \, \mathbb{E}\!\left[ \psi(W; \theta_0, \eta) \right] 
\right|_{\eta = \eta_0} = 0
$$
...
\theta_0&= \frac{E[\tilde{Y}\tilde{D}]}{E[\tilde{D^2}]}  \text{; true parameter derivation}\\
\hat{\theta} &= \frac{\sum_{i=1}^{N} \tilde{D}_i \tilde{Y}_i}{\sum_{i=1}^{N} \tilde{D}_i^2}  \text{; estimation}\\
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial}{\partial g} E[(Y-g(X) - \theta_0 (D-m_0(X))) \cdot (D - m(X)) \cdot h_g(X)]|_{g = g_0} &= 0 \\
\frac{\partial}{\partial m} E[(Y-g_0(X) - \theta_0 (D-m(X))) \cdot (D - m(X)) \cdot h_m(X)]|_{m = m_0} &= 0 \\
\end{aligned}
$$


where $h_g(X)$ and $h_m(X)$ are functional perturbations in nuisance function estimation. Specifically:

$$
\begin{aligned}
E[\frac{\partial \psi(W;\theta_0,g,m_0)}{\partial g} \cdot h_g] |_{g = g_0} &= 0 \\
&= E[\frac{\partial (Y-g(X) - \theta_0 (D-m_0(X))) \cdot (D - m(X)))}{\partial g} \cdot h_g] \\
&= E[-1 \cdot (D - m_0(X)) \cdot h_g] \\
\text{By Law of Iterated Expecations,} \\
&= E[E[(m_0(X) - D) \cdot h_g]|X]  \\
\text{Since } E[h_g(X)|X] = h_g(X), \\
&= E[h_g(X) \cdot E[m_0(X) - D|X]] \\
...
$$
\begin{aligned}
&\;E\Big[\frac{\partial \psi(W;\theta_0,g_0,m)}{\partial m}\cdot h_m\Big]\Big|_{m=m_0}
= E\Big[\frac{\partial\big((Y-g_0(X)-\theta_0(D-m(X)))(D-m(X))\big)}{\partial m}\cdot h_m\Big]_{m=m_0} \\
&\text{Let } A := Y-g_0(X)-\theta_0(D-m(X)),\quad B := D-m(X) \\
&\text{Then } \psi = A\cdot B,\quad 
\frac{\partial A}{ \partial m}= \theta_0\,\quad 
...
$$
\begin{aligned}
E[\frac{\partial \psi(W;\theta_0,g_0,m)}{\partial m} \cdot h_m] |_{m = m_0} &= E[h_m \cdot (-A + \theta_0 B)] \\
&= E[h_m \cdot (-Y + g_0(X) + 2\theta_0(D-m(X))] \\
\text{By Law of Iterated Expecations:} \\
&= E[E[h_m \cdot (-Y + g_0(X) + 2\theta_0(D-m(X))|X]] \\
\text{Since } E[h_m(X)|X] = h_m(X): \\
&= E[h_m \cdot \Big(E[-Y + g_0(X) + 2\theta_0(D-m(X)|X]\Big)]  \\
...
$$
\begin{aligned}
\psi(W;\hat{\theta}, \hat{g}, \hat{m}) &= \psi(W;\theta_0, g_0, m_0) + \frac{\partial \psi(W;\theta_0,g_0,m_0)}{\partial \theta} (\hat{\theta} - \theta_0) \\
&\quad + \frac{\partial \psi(W;\theta_0,g_0,m_0)}{\partial m} (\hat{m}-m_0) + \frac{\partial \psi(W;\theta_0,g_0,m_0)}{\partial g} (\hat{g}-g_0) + R \\
&= \psi(W;\theta_0, g_0, m_0) + \frac{\partial \psi(W;\theta_0,g_0,m_0)}{\partial \theta} (\hat{\theta} - \theta_0) \\
\text{By Neyman orthogonality,}\\
&\quad + 0 \cdot  (\hat{m}-m_0) + 0 \cdot (\hat{g}-g_0) + R \\
&= \psi(W;\theta_0, g_0, m_0) + \frac{\partial \psi(W;\theta_0,g_0,m_0)}{\partial \theta} (\hat{\theta} - \theta_0) + R\\
\end{aligned}
$$
...
$$
\begin{aligned}
0&=\psi(W;\theta_0, g_0, m_0) + \frac{\partial \psi(W;\theta_0,g_0,m_0)}{\partial \theta} (\hat{\theta} - \theta_0) + R\\
\frac{\partial \psi(\theta_0,g_0,m_0)}{\partial \theta} (\hat{\theta} - \theta_0) &= - \psi(\theta_0, g_0, m_0) - R \\
\hat{\theta} - \theta_0 &= \big[-\frac{\partial \psi(\theta_0,g_0,m_0)}{\partial \theta} \big]^{-1} \big[  \psi(\theta_0, g_0, m_0) + R\big] \\
\end{aligned}
$$

First, let us consider why $n^{\frac{1}{2}} \cdot (\hat{\theta} - \theta_0) \xrightarrow{distribution} \mathcal{N}(0, \Sigma)$. Let $C = \big[-\frac{\partial \psi(\theta_0,g_0,m_0)}{\partial \theta} \big]^{-1}$ be a constant matrix.
