...
After that, we perform log-transformed regression on the examples with non-zero outcome (`{python} f'{train_df_nonzero.shape[0]:,}'` in train). Without the need to worry about `ln(0)` outcome, the regression is much more straightforward albeit with fewer examples to train on.
