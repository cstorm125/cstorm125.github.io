...
1. Remove transactions without `CustomerID`; from `{python} original_nb` to `{python} has_cid_nb` transactions
2. Filter out transactions where either `UnitPrice` or `Quantity` is less than zero; from `{python} has_cid_nb` to `{python} has_sales_nb` transactions
